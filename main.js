...
var rgb_str = function(rgb_obj, alpha) {
  return 'rgba(' + rgb_obj.r + ',' + rgb_obj.g + ',' + rgb_obj.b + ',' + alpha + ')';
};



var magic_glow = function (src_img, color, alpha_factor) {
  ...
  gradient_innerglow.addColorStop(1.0, rgb_str(color_rgb, 0.9*alpha_factor));   //with transparency
  //draw
  ...
  gradient_linear.addColorStop(0.0, rgb_str({r:255,g:255,b:255}, 1*alpha_factor));
  gradient_linear.addColorStop(1.0, rgb_str({r:255,g:255,b:255}, 0.11*alpha_factor));
  //draw
  context.save();
  ...
  var elypse = function(cx, cy, rx, ry) {
    ...
    context.shadowColor = rgb_str(color_rgb, 0.5*alpha_factor);
